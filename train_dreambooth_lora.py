...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_images_path = [p for p in Path(instance_data_root).iterdir() if p.suffix != '.txt']
...
def main(args):
    ...
    for name, attn_processor in unet.attn_processors.items():
        ...
        unet_lora_attn_procs[name] = lora_attn_processor_class(
            hidden_size=hidden_size, cross_attention_dim=cross_attention_dim, rank=args.unet_rank
        )
        ...
        for name, module in text_encoder.named_modules():
            if any(x in name for x in TEXT_ENCODER_TARGET_MODULES):
                text_lora_attn_procs[name] = LoRAAttnProcessor(
                    hidden_size=module.out_features, cross_attention_dim=None, rank=args.unet_rank
                )
